...
class Google {
    static init(_passport) {
        _passport.use(new passport_google_oauth20_1.Strategy({
            clientID: index_1.default.clientID,
            clientSecret: index_1.default.clientSecret,
            callbackURL: index_1.default.callbackURL,
            passReqToCallback: true,
            scope: ['email', 'profile'],
        }, (req, accessToken, refreshToken, profile, done) => __awaiter(this, void 0, void 0, function* () {
            console.log(refreshToken, profile, "hiiii");
            const user = User_1.default.find(user => user.google === profile.id);
            if (user) {
                return done(null, user);
            }
            console.log(profile.id, profile.displayName);
            const newUser = (0, User_1.addUser)({
                google: profile.id,
